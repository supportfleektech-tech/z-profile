...
import { platformRoutes, type PageRoute } from '../../types/routes';
import { ArrowLeft, ChevronRight, Home, Share2, Check } from 'lucide-react';
...
export const PageLayout: React.FC<PageLayoutProps> = ({ children, title, badge = 'Live Production', relatedPages, actions }) => {
  const { navigate, goBack, currentPath } = useAppRouter();
  const { currentUser, can } = useAppData();
  ...
  /** Prev/Next only ever walk routes this tier can actually open. */
  const reachable = useMemo(
    () =>
      platformRoutes.filter(
        (r: PageRoute) => !r.public && currentUser && (!r.tiers || r.tiers.includes(currentUser.tier)) && (!r.permission || can(r.permission))
      ),
    [currentUser, can]
  );

  const currentIndex = reachable.findIndex((r) => r.path === currentPath);
  const prevRoute = currentIndex > 0 ? reachable[currentIndex - 1] : null;
  const nextRoute = currentIndex >= 0 && currentIndex < reachable.length - 1 ? reachable[currentIndex + 1] : null;
  ...
  return (
    <div className="w-full flex-1 flex flex-col min-h-0 bg-tech-grid">
      ...
      <div className="w-full bg-[#06101c]/95 border-b border-sky-950/80 px-3 sm:px-5 py-2 sm:py-2.5 no-print">
        <div className="max-w-[1600px] mx-auto flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            {badge && (
              <span className="hidden sm:flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 shrink-0">
            ...
            <button
              onClick={copyUrl}
              className="p-1.5 rounded-lg bg-sky-950/70 hover:bg-sky-900 text-slate-400 hover:text-white transition-colors text-xs flex items-center gap-1 shrink-0"
      ...
      {/* Body */}
      <div className="flex-1 w-full max-w-[1600px] mx-auto p-2 sm:p-4 md:p-5 flex flex-col gap-4 min-w-0">
        <div className="w-full bg-[#071120] border border-sky-900/50 rounded-xl sm:rounded-2xl shadow-[0_10px_35px_rgba(0,0,0,0.5)] overflow-hidden animate-slide-up min-w-0">
          {children}
        </div>
